...
def join_snails(snails_l: deque[Snail], snails_r: deque[Snail]) -> deque[Snail]:
    joined: deque[Snail] = deque()
    for snail in snails_l:
        joined.append(Snail(snail.val, snail.depth + 1))
    for snail in snails_r:
        joined.append(Snail(snail.val, snail.depth + 1))
    reduce_snail(joined)
    return joined
...
def solve(inp: str) -> int:
    ...
    max_mag = 0
    for a, b in it.combinations(snails, 2):
        mag1 = magnitude(join_snails(a, b))
        mag2 = magnitude(join_snails(b, a))
